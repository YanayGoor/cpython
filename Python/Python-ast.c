...
static int init_types(void)
{
    ...
    Pyx_type = make_type("Pyx", expr_type, Pyx_fields, 3);
...
expr_ty
Pyx(string name, asdl_seq * kwargs, asdl_seq * children, int lineno, int
    col_offset, PyArena *arena)
{
    expr_ty p;
    if (!name) {
...
ast2obj_expr(void* _o)
{
    ...
    case Pyx_kind:
        ...
        if (_PyObject_SetAttrId(result, &PyId_name, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_list(o->v.Pyx.kwargs, ast2obj_keyword);
        if (!value) goto failed;
...
obj2ast_expr(PyObject* obj, expr_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (_PyObject_LookupAttrId(obj, &PyId_kwargs, &tmp) < 0) {
            return 1;
        }
        if (tmp == NULL) {
            PyErr_SetString(PyExc_TypeError, "required field \"kwargs\" missing from Pyx");
            return 1;
        }
        else {
            int res;
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Pyx field \"kwargs\" must be a list, not a %.200s", tmp->ob_type->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
            kwargs = _Py_asdl_seq_new(len, arena);
            if (kwargs == NULL) goto failed;
            for (i = 0; i < len; i++) {
                keyword_ty val;
                res = obj2ast_keyword(PyList_GET_ITEM(tmp, i), &val, arena);
                if (res != 0) goto failed;
                if (len != PyList_GET_SIZE(tmp)) {
                    PyErr_SetString(PyExc_RuntimeError, "Pyx field \"kwargs\" changed size during iteration");
                    goto failed;
                }
                asdl_seq_SET(kwargs, i, val);
            }
            Py_CLEAR(tmp);
        }
        ...
        *out = Pyx(name, kwargs, children, lineno, col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
